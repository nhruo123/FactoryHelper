...
namespace FactoryHelper.Entities {
    ...
    public class ElectrifiedWall : FactorySpike {
        public static readonly ParticleType P_Fizzle = new() {
            Size = 1f,
        ...
        private static readonly float _particleEmittionPeriodVariance = 0.1f;
        ...
        private readonly Color[] _electricityColorsLerped;
        private readonly float _baseParticleEmittionPeriod;
        private readonly LedgeBlocker _ledgeBlocker;
        ...
        public ElectrifiedWall(Vector2 position, int size, Directions direction, string activationId, bool startActive)
            ...
            Vector2 offset = direction switch {
                Directions.Left or Directions.Right => Vector2.UnitY * size,
                _ => Vector2.UnitX * size,
            };
            _end = offset;
            ...
            switch (direction) {
                case Directions.Up:
                    Collider = new Hitbox(size, 6f, 0f, -3f);
                ...
                case Directions.Right:
                    Collider = new Hitbox(6f, size, -3f);
                    Add(_ledgeBlocker = new LedgeBlocker());
                    break;
            }
        }
        ...
        protected new void OnCollide(Player player) {
            if (Activator.IsOn) {
                Vector2 dir = Direction switch {
                    Directions.Left or Directions.Right => player.Center.X < Center.X ? -Vector2.UnitX : Vector2.UnitX,
                    _ => player.Center.Y < Center.Y ? -Vector2.UnitY : Vector2.UnitY,
                };
                player.Die(dir);
            }
        }
